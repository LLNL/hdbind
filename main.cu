...
int main(int argc, char* argv[]) {
    // ./main [TRAIN dataset path] [TEST dataset path] [DIM] [ITER] [Q] [Learning Rate]
    // Example:
    // ./main datasets/UCIHAR/UCIHAR_train.choir_dat datasets/UCIHAR/UCIHAR_test.choir_dat 10000 20 10 1
    ...
    std::vector<std::vector<float>> x_train_tmp;
    std::vector<int> y_train;
    std::vector<int> y_test;

    readChoirDat(argv[1], nFeatures_train, nClasses_train, x_train_tmp, y_train);
    readChoirDat(argv[2], nFeatures_test, nClasses_test, x_test_tmp, y_test);

    // normalize
    l2norm(x_train_tmp);
    l2norm(x_test_tmp);

    if (x_train_tmp.size() >= LIM) {
        x_train.assign(x_train_tmp.begin(), x_train_tmp.begin()+LIM);
    }
    else {
        x_train = x_train_tmp;
    }

    if (x_test_tmp.size() >= LIM) {
        x_test.assign(x_test_tmp.begin(), x_test_tmp.begin()+LIM);
    }
    else {
        x_test = x_test_tmp;
    }
...
#if defined(USE_IDNGRAM_ENCODING) || defined(USE_LVID_ENCODING)
    ...
    int id_stride = dim * 4;
    int fm_stride = nFeatures_train * 4;
    encodeIDNgram<<<encodeblocksTrain, encodeTPB>>>(d_hvs_train, d_level_hvs, d_id_hvs, d_x_train,
                                                    level_stride, id_stride, fm_stride, Q, nFeatures_train, 
                                                    3 /*ngramN*/, train_set_num /*dataN*/, dim);
#endif
    ...
    if (err != cudaSuccess)
    {
        ...
        fprintf(stderr, "Error name: (%s). Please check nvcc compile options.\n", err_name);
        exit(EXIT_FAILURE);
    }
...
#ifdef USE_IDNGRAM_ENCODING
    dim3 encodeblocksTest((dim + N_THREADS - 1) / N_THREADS, test_set_num);
    encodeIDNgram<<<encodeblocksTest, encodeTPB>>>(d_hvs_test, d_level_hvs, d_id_hvs, d_x_test,
                                                    level_stride, id_stride, fm_stride, Q, nFeatures_test, 
                                                    3 /*ngramN*/, test_set_num /*dataN*/, dim);
#endif
