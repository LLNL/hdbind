...
def random_partition_dude_data(data_file, output_dir):
    ...
    x_train, x_test = train_test_split(
        data, stratify=data[:, label_index], test_size=0.2
    )
...
def partition_lit_pcba_data(data_dir):
    ...
    actives_data = np.concatenate(
        [actives_data, np.ones((actives_data.shape[0], 1))], axis=1
    )

    inactives_data = np.concatenate(
        [inactives_data, np.zeros((inactives_data.shape[0], 1))], axis=1
    )
