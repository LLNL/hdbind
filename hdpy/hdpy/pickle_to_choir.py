...
def get_args():
    ...
    parser.add_argument(
        "--data", help="path that points to data pickle file", default="eeg1024.pickle"
    )
...
def main(args):
    ...
    if args.test_only:
        ...
        writeDataSetForChoirSIM(
            (X_test, y_test), f"{output_path}/{output_prefix}_test.choir_dat"
        )
        ...
        writeDataSetForChoirSIM(
            (X_train, y_train), f"{output_path}/{output_prefix}_train.choir_dat"
        )
        writeDataSetForChoirSIM(
            (X_test, y_test), f"{output_path}/{output_prefix}_test.choir_dat"
        )
