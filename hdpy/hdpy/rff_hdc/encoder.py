...
class LinearEncoder:
    ...
    def encode_data(self, datast):
        ...
        for i in range(n):
            ...
            rv[i] = self.encode_one_img((datast[i][0].view(-1)).int())  # transform to [0, 255]
            # labels[i] = datast[i][1]
            if (i % 1000 == 999): print("{} data points encoded".format(i + 1))
...
class RandomFourierEncoder:
    def __init__(self, input_dim, gamma, gorder=2, output_dim=10000, n_feat_buckets=100):
        ...
        self.n_feat_buckets = n_feat_buckets
    ...
    # NOTE: need to figure out how to get this out of the pixel value domain...256 possible values I know
    def build_item_mem(self):  # create random fourier features for 256 pixel values
        ...
        x = np.linspace(0, 1, num=2, dtype=np.float32)
        # Cov = np.array([np.exp(-correction_factor * self.gamma ** 2 * ((x - y) / 255.0) ** 2 / 2) for y in range(256)])
        
        Cov = np.array([np.exp(-correction_factor * self.gamma ** 2 * ((x - y) / 1.0) ** 2 / 2) for y in range(2)])
        #Entries in this Cov matrix are generated by the RBF kernel https://en.wikipedia.org/wiki/Radial_basis_function_kernel
    ...
    def encode_one_img(self, x):
        ...
        batch_size, channels, num_pixels = x.size()
        # rv = self.item_mem[x].view(batch_size, channels, num_pixels, -1).transpose(0, 2)

        # rv = self.item_mem[x].view(batch_size, channels, num_pixels, -1)
        rv = self.item_mem[x.flatten()].view(batch_size, channels, num_pixels, -1).transpose(0, 2)
        ...
        return rv.transpose(0,1).reshape((batch_size, -1))
    ...
    def encode_data(self, X):
        #todo: remember that black and white images (e.g. MNIST) have pixel values ranging from 0 to 255 and this code is littered 
        # with things specific to that case
        '''
        ...
        return: rv -> hypervectors for data point. shape=[N, HDC dim]
        '''


        '''
        # channels = datast[0][0].size(0)
        n = len(X)  # number of examples in x
        # rv = torch.zeros((n, channels * self.output_dim))



        rv = torch.zeros((n, self.output_dim))
        # labels = torch.zeros(n).long()
        ...
        for i, batch in enumerate(data_loader):
            # cur_batch_size = batch[0].size(0)  # in case the last batch is not equal to batch_size
            print(i)
            rv[i] = self.encode_one_img(batch[i].view(1, -1)).int()
            # rv[i * batch_size: i * batch_size + cur_batch_size] = self.encode_one_img(
                # (batch[0].view(cur_batch_size, -1)).int())
            # labels[i * batch_size: i * batch_size + num_imgs] = batch_img[1]
            if i % 100 == 99: print(
                f"{(i + 1) * batch_size} data points encoded. Total time elapse = {time.time() - start_time}")
        ...
        batch_size = 128
        data_loader = torch.utils.data.DataLoader(X, batch_size=batch_size, shuffle=False)
        for i, batch_img in tqdm(enumerate(data_loader), total=len(data_loader)):
            num_imgs = batch_img.size(0)  # in case the last batch is not equal to batch_size
            rv[i * batch_size: i * batch_size + num_imgs] = self.encode_one_img(
                ((1) * batch_img.view(num_imgs, channels, -1)).int())
            if i % 100 == 99: print(
