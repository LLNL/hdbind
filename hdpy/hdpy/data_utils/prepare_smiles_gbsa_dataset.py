...
for path in p.glob("*_gbsa_smiles_with_base_rdkit_smiles.csv"):
	...
	print(f"{name}:\t{path}")
	output_dir = f"/usr/workspace/atom/gbsa_modeling/dude_smiles/deepchem_feats_labeled_by_gbsa"
	# for feat in ["ecfp", "smiles_to_seq", "smiles_to_image", "coul_matrix", "mordred", "maacs", "rdkit"]:
	for feat in ["ecfp", "smiles_to_seq", "smiles_to_image", "mordred", "maacs", "rdkit"]:
