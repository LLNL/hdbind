...
def parse_args():
    ...
    parser.add_argument('--test-data', type=str,
            help='choir testing dataset')

    parser.add_argument('-d', '--dimensions', default=10000, type=int,
            required=False, help='sets dimensions value', dest='dimensions')

    parser.add_argument('-q', '--quantization', default=10, type=int,
            required=False, help='sets quantization level', dest='Q')

    parser.add_argument('-b', '--batchsize', default=32, type=int,
            required=False, help='sets batch size', dest='batch_size')
    
    parser.add_argument('-i', '--iterations', default=20, type=int,
            required=False, help='sets iteration number', dest='iterations')

    parser.add_argument('-s', '--sim_metric', default='cos', type=str,
            required=False, help='sets similarity metric', dest='sim_metric',
            choices=['dot', 'cos'])

    parser.add_argument('-r', '--randomseed', default=0, type=int,
            required=False, help='sets random seed', dest='random_seed') 
...
def encode_tf(N, Q, level_hvs, id_hvs, feature_matrix):
    ...
    with tf.device('/gpu:0'):
        encode_sample = lambda i: \
            tf.reduce_sum(input_tensor=tf.multiply(
                tf.gather(level_hvs,
                    tf.cast(tf.scalar_mul(Q,
        ...
        with timing_part(str(encode_tf)):
            tf_hv_matrix_final = tf.while_loop(cond=cond, body=body, loop_vars=(0, tf_hv_matrix))[1]
            #writer = tf.summary.FileWriter('./graphs', sess.graph)
            return tf.reshape(tf_hv_matrix_final.stack(), (feature_matrix.shape[0], level_hvs.shape[1]))
...
def encode(x, x_test, D, Q):
    ...
    for q in range(Q+1):
        ...
        level_hv[:flip] = level_base[:flip] * -1.
        ...
        with timing_part(str(retrain)):
            with tf.device('/gpu:0'):
                for i, _y in enumerate(y):
                    sims = tf.linalg.matvec(model, x_h[i]) / tf.norm(model) #tf.norm(x_h[i]) 
            ...
            with tf.device('/gpu:0'):
                for i in range(0, N, B):
                    ...
                    for c in range(K):
                        ...
                        mlist[c] += tf.reduce_sum(tf.boolean_mask(x_batch, corct_mask), 0)
                        mlist[c] -= tf.reduce_sum(tf.boolean_mask(x_batch, wrong_mask), 0)
                    model = tf.stack(mlist)
...
def load_pdbbind_from_hdf(hdf_path, dataset_name, train_split_path, test_split_path, bind_thresh, no_bind_thresh):
...
def main():
    ...
    if args.filename.endswith('_train.choir_dat'):
        tst_filename = args.filename.replace(
                '_train.choir_dat', '_test.choir_dat')
        x_test, y_test, _ = load_with_verbose_info(tst_filename)
        n_test = len(x_test)
    '''
    ...
    dummy_class_model_uniform = DummyClassifier(strategy='uniform', random_state=args.random_seed)
    dummy_class_model_most_frequent = DummyClassifier(strategy='most_frequent', random_state=args.random_seed)
