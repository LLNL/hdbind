...
def test(MODEL, loader, criterion, device, model_='rff-hdc'):
    ...
    print(f'\n{classification_report(y_true=torch.cat(label_list).view(-1).cpu(), y_pred=torch.cat(pred_list).view(-1).cpu())}')
...
def train(args):
    ...
    for epoch in range(args.epoch):
        ...
        print(f'train time: {timer.total_time}')
        print('Start testing on test set')
        with timing_part("TEST-RFF") as timer:
            test(model, testloader, criterion, device, model_=args.model)
        print(f'test time: {timer.total_time}')
...
def argument_parser():
    ...
    parser.add_argument('-dataset', type=str, choices=['mnist', 'fmnist', 'cifar', 'isolet', 'ucihar', 'dude'], default='mnist',
                        help='dataset (mnist | fmnist | cifar | isolet | ucihar | dude)')
    parser.add_argument('--dude-path', help="path to the dude dataset of choice")
