...
def ecfp_job(smiles):
    ...
    try:
        ...
        if mol is not None:
            fp_start = time.time()
            fp_vec = AllChem.GetMorganFingerprintAsBitVect(mol, args.ecfp_radius, nBits=args.ecfp_length)
            fp_end = time.time()
            ...
            return {"ecfp": fp, "smiles_to_mol_time": mol_end - mol_start, 
                    "mol_to_fp_time": fp_end - fp_start}
            # return fp_vec
        else:
            return None
...
def rp_ecfp_hdc(ecfp_list):
    ...
    if args.debug:
        import pdb
        pdb.set_trace()
    return torch.cat(encoding_list), (sum(ecfp_encode_time_list) / args.n_trials)/ len(ecfp_list)


def ecfp_timing(pool, smiles_list):

    result_list = list(tqdm(pool.imap(ecfp_job, smiles_list), total=len(smiles_list), desc="computing ECFPs"))
...
def smiles_token_hdc(smiles_list):
    ...
    if args.tokenizer == "bpe":
        ...
        print(f"MoleHD-inspired baseline, tokenizer={args.tokenizer}-{args.ngram_order}")
    
    dataset = SMILESDataset(smiles=smiles_list, 
    ...
    tok_encoder = TokenEncoder(D=args.dimension, 
                            num_classes=2, 
                            item_mem=dataset.item_mem)
...
def lit_pcba_main():
    ...
    path = Path("/g/g13/jones289/workspace/hd-cuda-master/datasets/lit_pcba/AVE_unbiased/ALDH1/")
    ...
    print(f"processing {path} with {df.shape[0]} SMILES strings. subsampling {df.shape[0]}")

    #MoleHD-based implementation
    molehd_mean_encode_time = smiles_token_hdc(smiles_list=target_smiles)


    ecfp_list, ecfp_timing_list, smiles_to_mol_time_list = ecfp_timing(pool=pool, smiles_list=target_smiles)

    # random projection encoding
    rp_ecfp_encodings, hdbind_rp_ecfp_mean_encode_time = rp_ecfp_hdc(ecfp_list=ecfp_list)
    

    print(f"MoleHD ({args.tokenizer}) encoded {len(df)} molecules at a rate of {molehd_mean_encode_time} seconds/molecule")
    print(f"HDBind-RP-ECFP encoded {len(df)} molecules at a rate of {hdbind_rp_ecfp_mean_encode_time} seconds/molecule")

    print(f"HDBind-RP-ECFP encoding speedup to MoleHD (bpe) is approximately {molehd_mean_encode_time/hdbind_rp_ecfp_mean_encode_time:0.2f}X")

    # import pdb
    # pdb.set_trace()

    rp_ecfp_dataset_train = TensorDataset(rp_ecfp_encodings[list(range(len(train_df))), :],
                                          torch.from_numpy(train_df["label"].values)) 
    rp_ecfp_dataset_test = TensorDataset(rp_ecfp_encodings[list(range(len(train_df), len(train_df)+len(test_df))), :],
                                          torch.from_numpy(test_df["label"].values))
    
    rp_ecfp_dataloader_train = DataLoader(rp_ecfp_dataset_train,
                                          num_workers=args.num_workers,
                                          batch_size=args.batch_size)
    rp_ecfp_dataloader_test = DataLoader(rp_ecfp_dataset_test,
                                         num_workers=args.num_workers,
                                         batch_size=args.batch_size)


    ecfp_dataset_train = TensorDataset(torch.from_numpy(train_npy[:,:-1]),
                                        torch.from_numpy(train_df["label"].values))
    ecfp_dataset_test = TensorDataset(torch.from_numpy(test_npy[:,:-1]),
                                        torch.from_numpy(test_df["label"].values))
    
    ecfp_dataloader_train = DataLoader(ecfp_dataset_train, 
                                       num_workers=args.num_workers, 
                                       batch_size=args.batch_size)
    ecfp_dataloader_test = DataLoader(ecfp_dataset_test, 
                                       num_workers=args.num_workers, 
                                       batch_size=args.batch_size)
    #train HDBind-rp-ecfp
    rp_ecfp_model=RPEncoder(D=args.dimension,input_size=args.ecfp_length, num_classes=2)
    train_hdc_no_encode(model=rp_ecfp_model,
              train_dataloader=rp_ecfp_dataloader_train,
              device="cuda", num_epochs=10)
    rp_test_dict = test_hdc(model=rp_ecfp_model,
             test_dataloader=ecfp_dataloader_test,
             device="cuda")
    ...
    mlp_model = MLPClassifier(layer_sizes=[(1024,512), (512,256),(256,128),(128,2)],
                              lr=1e-3,
    ...
    mlp_train_dict = train_mlp(model=mlp_model, train_dataloader=ecfp_dataloader_train, epochs=10, device="cuda")
    mlp_test_dict = val_mlp(model=mlp_train_dict["model"], val_dataloader=ecfp_dataloader_test, device="cuda")
    ...
    print(f"HDBind-RP-ECFP prediction speedup vs MLP: {mlp_test_dict['forward_time']/rp_test_dict['test_time']:.2f}X")
