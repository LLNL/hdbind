...
__global__ void cosineSimilarityVec_woreuse(float* result, float* classHVs, int dim, int colNum, float* norm_2_const, int dataidx) {
    float norm_2 = norm_2_const[dataidx];
    int idx = threadIdx.x + blockDim.x * blockIdx.x;
    if (idx < colNum)
        result[idx] = result[idx] / (normf(dim, classHVs + idx * dim) * norm_2);
}
